...
BEGIN
  ...
  IF NEW.state = 0 OR NEW.state = 10 THEN
    ...
    IF NOT ARRAY['ks'] <@ dblink_get_connections() OR dblink_get_connections() IS NULL THEN
      PERFORM dblink_connect('ks', 'kamailio_server');
    END IF;
    ...
    IF NEW.state = 10 THEN
      pwd := substring(MD5(gen_random_bytes(128)) from 0 for 25);
