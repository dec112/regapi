...
function registerDevice(req, res, next, swagger) {
    ...
    device.owner_token = (config.api.verify_owner ? tools.strRandom(8) : 'IGNORED');
    device.phone_token = (config.api.verify_phone ? tools.strRandom(8) : 'IGNORED');
    device.email_token = (config.api.verify_email ? tools.strRandom(8) : 'IGNORED');
...
function unregisterDevice(req, res, next, swagger) {
    ...
    if(!device_id)
        return next(swagger.errors.notFound('device_id'));
    if(config.api.delete_unregistered)
            ...
            return db.one('SELECT * FROM ' + SCHEMA + 'subscriber ' +
                ...
                .then(function (data) {
                    ...
                    result = {
                        code: 200,
                        device_id: device_id,
                        server: config.kamailio.ws,
                        publicId: "sip:" + data.username + "@" + config.kamailio.domain,
                        privateId: data.username,
                        password: data.password,
                        realm: data.domain,
                        services: config.kamailio.services
                    };
                    tools.logDebug(method + ' response', result);
                    if(config.debug)
                        result.runtime_ms = tools.elapsedHrTime(start);
